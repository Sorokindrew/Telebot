...
def get_region_id(msg: Message):
    ...
    if not list_of_cities:
        ...
        for city in list_of_cities:
            markup.add(
                InlineKeyboardButton(city[0],
                                     callback_data=city[0] + '|' + city[1] +
                                                   '|' +
        ...
        for hotel in api.get_list_of_hotels(data):
            if data['is_photos_enabled'] == 'No':
                ...
                for index in range(data['quantity_of_photos']):
                    with open(f'hotels/{hotel.get_name()}/img{index}.jpg',
                              'rb') as f:
                        if index == 0:
                            media.append(
                                InputMediaPhoto(f.read(),
                                                caption=hotel.get_hotel_info()))
                        else:
                            media.append(InputMediaPhoto(f.read()))
                shutil.rmtree(f'hotels/{hotel.get_name()}')
